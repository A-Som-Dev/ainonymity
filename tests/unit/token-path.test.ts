...
describe('ensureTokenDir', () => {
  ...
  it('creates the .ainonymous directory on win32 if missing', () => {
    const fakeWinPath = join(testHome, '.ainonymous', 'ainonymous-8600.token');
    ensureTokenDir(fakeWinPath, 'win32');
    expect(existsSync(join(testHome, '.ainonymous'))).toBe(true);
  });

  it('is idempotent on win32', () => {
    const fakeWinPath = join(testHome, '.ainonymous', 'ainonymous-8700.token');
    ensureTokenDir(fakeWinPath, 'win32');
    expect(() => ensureTokenDir(fakeWinPath, 'win32')).not.toThrow();
    expect(existsSync(join(testHome, '.ainonymous'))).toBe(true);
  });

  it('sets restrictive mode on the .ainonymous dir on win32', () => {
    const fakeWinPath = join(testHome, '.ainonymous', 'ainonymous-8800.token');
    ensureTokenDir(fakeWinPath, 'win32');
    const dirStat = statSync(join(testHome, '.ainonymous'));
...
describe('hardenTokenFileAcl', () => {
  it('is a no-op on linux and reports success', () => {
    const result = hardenTokenFileAcl('/tmp/ainonymous-8100.token', 'linux');
    expect(result).toBe(true);
  });

  it('is a no-op on darwin and reports success', () => {
    const result = hardenTokenFileAcl('/tmp/ainonymous-8200.token', 'darwin');
    expect(result).toBe(true);
  });

  it('returns false on win32 when no username can be determined', () => {
    const result = hardenTokenFileAcl('C:\\nonexistent\\ainonymous-8300.token', 'win32', {
      USERNAME: '',
    });
