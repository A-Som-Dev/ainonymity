...
describe('createProxyServer', () => {
  ...
  it('responds to /dashboard with html and strict CSP', async () => {
    ...
    expect(body).toContain('AInonymous');
...
describe('management endpoint auth', () => {
  ...
  let server: Server | null = null;
  const ORIG_ENV = process.env['AINONYMOUS_MGMT_TOKEN'];

  beforeEach(() => {
    delete process.env['AINONYMOUS_MGMT_TOKEN'];
  });
  ...
  afterEach(() => {
    ...
    if (ORIG_ENV === undefined) {
      ...
      process.env['AINONYMOUS_MGMT_TOKEN'] = ORIG_ENV;
    }
  });
  ...
  it('lets env var AINONYMOUS_MGMT_TOKEN override options', async () => {
    process.env['AINONYMOUS_MGMT_TOKEN'] = 'env-token-ABCDEFGHIJ';
