...
| I | Pseudonym collisions between unrelated originals. | `partial` | `PseudoGen` cycles 24 greek letters then appends numeric suffixes. Collisions within a single session are prevented by the BiMap (same original -> same pseudonym). Cross-session collisions (restart creates a new mapping) are accepted; same pseudonym may map to different originals across sessions. |
