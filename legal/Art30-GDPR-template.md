...
**Hinweis zur Praxis**: Bei dauerhafter Nutzung von LLM-Diensten im Rahmen der Softwareentwicklung, Analyse oder Kundenkommunikation, auch wenn die Übermittlung anonymisiert via AInonymous-Proxy erfolgt, liegt regelmäßig eine nicht nur gelegentliche Verarbeitung vor. Die Verzeichnispflicht besteht in diesen Fällen unabhängig von der Mitarbeiterzahl.

---

## Eintrag: _[Bezeichnung der Verarbeitung, z.B. "LLM-gestützte Code-Analyse via AInonymous-Proxy"]_
...
- **Zusätzliche Schutzmaßnahmen** (Transfer Impact Assessment nach Schrems II): Anonymisierung und Pseudonymisierung durch AInonymous vor Transfer; Entfernung von Secrets (API-Keys, Tokens, Passwörter); konfigurierbare Retention-Policies beim Empfänger.
...
- Eine jährliche Review wird empfohlen; bei wesentlichen Änderungen an AInonymous (Version-Upgrades, neue Detection-Module, geänderte Pipeline) zusätzliche Ad-hoc-Prüfung.
